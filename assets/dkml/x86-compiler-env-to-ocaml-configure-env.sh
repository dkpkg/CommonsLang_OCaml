...
# is the plain assembler. ASPP is not in the launcher's fixed variable list,
# so only export_binding reaches it (a plain ASPP= assignment is a silent
# no-op that falls back to configure's ASPP="$CC -c" default; the same value
# here, but by luck, not mechanism).
autodetect_compiler_AS="i686-none-linux-gnu-as"
autodetect_compiler_ASFLAGS=""
export_binding ASPP "i686-none-linux-gnu-gcc -c"
