local M = {
  ...
  -- prompts and suggested `trust grant` commands, so a consumer grants a
  -- rule's full need in one command instead of one denial at a time. The
  -- sets are the reachable request.ui.capture ("run") and
  -- request.ui.writefile ("write") calls of each rule, including the shared
  -- H.run/H.write_projfile helpers and cross-rule proxies (GenerateForms
  -- drives the three Generate* rules; Adopt and Refresh drive Solve).
  uirule_capabilities = {
    Solve = { "run", "write" },
    GenerateDriver = { "write" },
    GenerateSrc = { "write" },
    GenerateFinal = { "write" },
    GenerateForms = { "write" },
    Adopt = { "run", "write" },
...
-- 1.1.21 lets OpamVenv select a released merged prefix by form id: `formid=`
-- names the get-object target directly (the driver discovery is skipped) and
-- `lock=` names the lock that prefix was solved from, so the stamp records its
-- sha256 and a lock change forces a rebuild. It also gathers the project's
-- declared environment modifications (`dk-opam-venv.envmods`, one `+NAME=VALUE`,
-- `<NAME=VALUE` or `-NAME` per line, planned by the engine's `envmod` library)
-- into env.ps1, env.cmd and env.sh, after the fixed toolchain block. A project
-- with no such file gets the same activators as before.

-- 1.1.20 speeds up Solve's pinned-version universe guard
-- (Apparatus.OpamLockHelper@1.0.19): the guard ran `opam list --all-versions`
-- once per pinned package, and each opam invocation reloads the switch state,
-- so a large pin table (MlFront: ~200 pins) dominated the solve wall-clock.
-- It now enumerates the universe once and checks the pins against a hash set
-- in memory, keeping the same FATAL "pin not in universe" diagnostic. Solve
-- output is unchanged: this is a pure speedup, no lock-format effect.

-- 1.1.19 stops emitting the source.incache field
-- (Apparatus.OpamLockHelper@1.0.18): incache was a solve-time cache
-- observation (always 0, present only for archives not in the local opam
-- download cache), so it varied run to run and made the committed lock
-- nondeterministic. The helper still uses the flag internally to select where
-- `size` is read. This is the sibling of the @1.1.18 local_opam_dir fix; the
-- two together make the emitted lock fully machine-independent.

-- 1.1.18 stamps generated.local_opam_dir only when the value is a relative
-- path (Apparatus.OpamLockHelper@1.0.17): a relative dir is a stable project
-- path the mode=solve replay can reuse, while an absolute dir is a transient
-- staging directory whose machine-specific path made the committed lock
-- nondeterministic. solve_user's replay reader is unchanged (a missing member
-- already falls back to the caller's local_opam_dir= or the "." default).

-- 1.1.17 makes the import gate dependency-environment-aware. GenerateDriver
-- imports a prebuilt package from a source only when the source's lock and this
-- lock agree on the package's version AND its resolved direct `depends`
-- name-set (with in-closure depopts folded into `depends` by Solve) AND,
-- recursively, the same for every package in its dependency closure; otherwise
-- the package is built locally. This closes the stale-import defect where
-- adding an optional dependency to a consumer left a prebuilt that had been
-- built WITHOUT that dependency still imported (opam would have rebuilt it).
-- See dk.u "## Resolution of imports". No lock-format or rule-shape change: a
-- source lock too old to record a comparable `depends` simply compares unequal
-- and the package builds locally (fail-safe, never a wrong import).

-- 1.1.15 declares the capability manifest above; no behavioral change to any
-- rule body (the version-unique helper table is renamed as usual).
...
CommonsLang_OCaml__Dk_OpamLock__1_1_21 = {}
...
CommonsLang_OCaml__Dk_OpamLock__1_1_21.MODVER = M.id
local mv_at = nil
local mv_i = 1
...
CommonsLang_OCaml__Dk_OpamLock__1_1_21.MODULE = string.sub(M.id, 1, mv_at - 1)
CommonsLang_OCaml__Dk_OpamLock__1_1_21.VERSION = string.sub(M.id, mv_at + 1)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.iswhite(c)
...
-- switch).
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.trim(s)
  if s == nil then return "" end
  local n = string.len(s)
  local a = 1
  while a <= n and CommonsLang_OCaml__Dk_OpamLock__1_1_21.iswhite(string.sub(s, a, a)) do a = a + 1 end
  if a > n then return "" end
  local b = n
  while b >= 1 and CommonsLang_OCaml__Dk_OpamLock__1_1_21.iswhite(string.sub(s, b, b)) do b = b - 1 end
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.join(tbl, sep)
  local r = nil
  local i = 1
  while tbl[i] ~= nil do
    if r == nil then
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.set_from_list(tbl)
  local set = {}
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.dirname(p)
...
function uirules.Solve(command, request, continue_)
  ...
  if continue_ ~= "solve" then
    ...
    local files = {
      helper = "$(get-asset CommonsLang_OCaml.Apparatus.OpamLockHelper@1.0.19 -p assets/opam-lock/dk_opam_lock.ml -f dk_opam_lock.ml)",
...
CommonsLang_OCaml__Dk_OpamLock__1_1_21.Solve = uirules.Solve
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.indexof_char(s, ch)
  local i = 1
  local n = string.len(s)
  while i <= n do
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.numstr(v)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.modsegment(name)
  local out = ""
  local i = 1
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.is_host_tool(name)
...
CommonsLang_OCaml__Dk_OpamLock__1_1_21.DKML_PROVIDED = {
...
CommonsLang_OCaml__Dk_OpamLock__1_1_21.DKML_SLOTS = {
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.driver_visit(byname, provided, name, seen, order)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
-- Order-insensitive equality of two dependency name arrays (a and b may be nil,
-- treated as empty). Compared as sets: `depends` is discovery-ordered in the
-- lock and its order can differ across opam-repository commits, so only set
-- membership is meaningful. A duplicated name collapses in the set (harmless).
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.set_equal(a, b)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
  local sa = H.set_from_list(a)
  local sb = H.set_from_list(b)
  local k = next(sa)
  while k do
    if sb[k] == nil then return "f" end
    k = next(sa, k)
  end
  k = next(sb)
  while k do
    if sa[k] == nil then return "f" end
    k = next(sb, k)
  end
  return "t"
end

-- Is package `name` consumable as a prebuilt from an import source, given the
-- consumer's lock (byname: name -> entry, byver: name -> version) and that
-- source's lock (sbyver: name -> version, sbydep: name -> depends array)?
-- Returns "t"/"f", memoized per source in `memo`; `why[name]` records the first
-- reason a name is rejected, for the caller's diagnostic. The rule mirrors
-- opam's rebuild-on-environment-change: a prebuilt is consumable only when the
-- source built it at the consumer's version, against the consumer's resolved
-- direct `depends` name-set (in-closure depopts already folded into `depends`
-- by Solve, so a newly present/absent optional dependency shows here as a set
-- change), and recursively against a consumable closure -- a dependency the
-- consumer now builds locally in a different environment is ABI-incompatible
-- with a prebuilt that linked the source's dependency. `provided` (toolchain or
-- virtual) packages are outside the comparison; the compiler is gated
-- separately by the `ocaml` string. `memo[name]` is set to "t" before recursing
-- so a pathological cycle terminates optimistically (opam graphs are acyclic,
-- matching driver_visit's no-cycle-check precedent).
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.import_compat(byname, byver, sbyver, sbydep, provided, name, memo, why)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
  if memo[name] ~= nil then return memo[name] end
  if provided[name] ~= nil then memo[name] = "t"; return "t" end
  local cver = byver[name]
  if cver == nil then why[name] = "absent from the consumer lock"; memo[name] = "f"; return "f" end
  if sbyver[name] == nil then why[name] = "absent from the import source"; memo[name] = "f"; return "f" end
  if sbyver[name] ~= cver then
    why[name] = "version " .. sbyver[name] .. " in the source vs " .. cver
    memo[name] = "f"; return "f"
  end
  memo[name] = "t"
  local cdeps = nil
  if byname[name] ~= nil then cdeps = byname[name].depends end
  if H.set_equal(cdeps, sbydep[name]) ~= "t" then
    why[name] = "the resolved depends set differs"
    memo[name] = "f"; return "f"
  end
  local i = 1
  while cdeps ~= nil and cdeps[i] ~= nil do
    local d = cdeps[i]
    if H.import_compat(byname, byver, sbyver, sbydep, provided, d, memo, why) == "f" then
      why[name] = "dependency " .. d .. " is not consumable"
      memo[name] = "f"; return "f"
    end
    i = i + 1
  end
...
--
-- Parameters (dk0 dialog H.MODULE .. ".GenerateDriver@" .. H.VERSION):
...
--                      CommonsLang_OCaml.Dk.OpamBuild.F_BuildLockedClosure@1.0.26,
...
function uirules.GenerateDriver(command, request)
  ...
  if rulefn == nil then rulefn = "CommonsLang_OCaml.Dk.OpamBuild.F_BuildLockedClosure@1.0.26" end
  ...
  local byname = {}
  local byver = {}   -- bare opam name -> version (for the import compatibility gate)
  local k = next(lock.packages)
  while k do
    local dot = H.indexof_char(k, ".")
    if dot ~= nil then
      local bn = string.sub(k, 1, dot - 1)
      byname[bn] = lock.packages[k]
      byver[bn] = string.sub(k, dot + 1)
    end
  ...
  -- the first source that was built with the same `ocaml` compiler as this lock,
  -- pins the package at the same opam version, and (via H.import_compat) matches
  ...
  if implibs ~= nil then
    ...
    while implibs[xi] ~= nil do
      ...
      if socaml == gate_ocaml then
        local sbyver = {}
        local sbydep = {}   -- source's bare name -> its depends array
        local sk = next(slock.packages)
        while sk do
          local sd = H.indexof_char(sk, ".")
          if sd ~= nil then
            local sn = string.sub(sk, 1, sd - 1)
            sbyver[sn] = string.sub(sk, sd + 1)
            sbydep[sn] = slock.packages[sk].depends
          end
          sk = next(slock.packages, sk)
        end
        -- Memoized per source: import_compat is a function of (this lock, this
        -- source), so its results and rejection reasons are reused across the
        -- package walk. env_local collects packages that pin at the right version
        -- but whose dependency environment differs, for the diagnostic below.
        local memo = {}
        local why = {}
        local env_local = {}
        ...
        while gk do
          ...
          if gd ~= nil then
            ...
            if imported[gname] == nil and sbyver[gname] == gver
              ...
              if H.import_compat(byname, byver, sbyver, sbydep, provided, gname, memo, why) == "t" then
                ...
                -- Same version, but the build-time dependency environment differs;
                -- consuming the prebuilt would be stale, so build it locally.
                local rz = why[gname]
                if rz == nil then rz = "the dependency environment differs" end
                table.insert(env_local, gname .. " (" .. rz .. ")")
              end
            end
          end
          gk = next(lock.packages, gk)
        end
        if env_local[1] ~= nil then
          print(H.MODVER .. ": " .. slib .. ": " .. H.numstr(H.alen(env_local)) ..
            " package(s) built locally because the dependency environment differs" ..
            " from the import source: " .. H.join(env_local, ", "))
        end
      end
      xi = xi + 1
    end
  ...
  elseif mergedprefix ~= nil then
    ...
    -- Deterministic in-process archive (dk 2.4.2.342 or later): same inputs,
    -- same prefix.zip bytes on every platform.
    table.insert(fcmds, "          [" .. nl .. "            \"--zip\", \"${SLOT.request}/prefix.zip\", \"-srcdir\", \"p\"" .. nl .. "          ]")
  else
...
CommonsLang_OCaml__Dk_OpamLock__1_1_21.GenerateDriver = uirules.GenerateDriver

-- Append each element of `list` (nil-safe) to the arg list `rb` as
-- "name[]=value" strings, for the "Regenerate with:" banner.
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.append_list_args(rb, name, list)
  local i = 1
...
function uirules.GenerateSrc(command, request)
  ...
  -- The in-process `--zip` special form (dk 2.4.2.342 or later) archives the
  -- assembly deterministically -- same source tree, same output.zip bytes on
  -- every platform -- and drops the emitted form's only S7z dependency (7zz
  -- stored fresh-extraction mtimes, so every rebuild of identical content
  -- produced distinct archive bytes). The bare SRCFILE keeps the
  -- `<root>-<version>/` entry prefix.
  table.insert(cmds, "          [" .. nl .. "            \"--zip\", \"${SLOT.request}/output.zip\", \"" .. assembly .. "\"" .. nl .. "          ]")
...
-- Cross-call binding; see the note at H.Solve.
CommonsLang_OCaml__Dk_OpamLock__1_1_21.GenerateSrc = uirules.GenerateSrc
...
-- Cross-call binding; see the note at H.Solve.
CommonsLang_OCaml__Dk_OpamLock__1_1_21.GenerateFinal = uirules.GenerateFinal
...
-- Cross-call binding; see the note at H.Solve.
CommonsLang_OCaml__Dk_OpamLock__1_1_21.GenerateForms = uirules.GenerateForms
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.is_exe_literal(e)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.exe_outname(e)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.check_segment(name, what)
  local bad = nil
  local n = string.len(name)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.check_modpath(path, what)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
  local rest = path
...
-- dune-project.
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.scrape_paren(text, key)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.default_pins()
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.indexof_str(s, sub, from)
  local n = string.len(s)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.replace_all(s, old, new)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.toint(s)
  local n = 0
  local i = 1
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.id_ver(id)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.vparts(ver)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
  local parts = {}
  local cur = ""
  local i = 1
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.vcmp(a, b)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
  local pa = H.vparts(a)
  local pb = H.vparts(b)
  local i = 1
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.import_max(text, prefix)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
  local needle = prefix .. "@"
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.rule_family(id)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.dku_import_version(text)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.import_wants(request)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.read_stamp(drivertext)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.first_rulefn(text)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.alen(t)
  local i = 0
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.kv(line, key)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.recover_hosttoolabi(privlist)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
  local i = 1
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.pkgset(text)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
  local set = {}
  local n = string.len(text)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.same_set(a, b)
  local k = next(a)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.reversion(id, newver)
  local i = string.len(id)
  while i >= 1 do
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.splitlines(s)
  local lines = {}
  local cur = ""
  local i = 1
  local n = string.len(s)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.ends_with(s, suf)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.fallback_params(text)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.stale_msg(D, got, want)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.discover_drivers(request, coreutils)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.solve_user(request)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
  local lockpath = request.user.lock or "dk.opam-lock.jsonc"
  local content = assert(request.ui.readfile { path = lockpath },
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.refresh_one(request, D, T, versionoverride, targettool)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
...
function uirules.Refresh(command, request, continue_)
  ...
  if T.pkg == nil and T.closure == nil then
    T.pkg = "CommonsLang_OCaml.Dk.OpamBuild.F_BuildLockedPackage@1.0.26"
    T.closure = "CommonsLang_OCaml.Dk.OpamBuild.F_BuildLockedClosure@1.0.26"
  end
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.to_slash(s)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.json_esc(s)
  local H = CommonsLang_OCaml__Dk_OpamLock__1_1_21
  return H.replace_all(H.replace_all(s, "\\", "\\\\"), "\"", "\\\"")
end

-- Backslash form (for Windows PATH entries baked into env.ps1/env.cmd).
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.to_back(s)
  return CommonsLang_OCaml__Dk_OpamLock__1_1_21.replace_all(s, "/", "\\")
end
...
-- the resolved path.
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.write_projfile(request, path, content)
...
function CommonsLang_OCaml__Dk_OpamLock__1_1_21.run(request, program, args, what)
